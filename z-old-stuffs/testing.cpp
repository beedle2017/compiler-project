...
void parseInputLine(DFA* dfa, Tokens* tokens_object, string inputLine, int lineNumber){
    ...
    for(int i=0; i<inputLine.length(); i++){
        ...
        if(currentChar == ' ' || currentChar == '\t' || currentChar == '\n'){
            ...
            // i--;
            //cout<<"zzz/n";
        } else if(lastChar != '\0' && currentWord.length() > 0 && ( (contains(alphabets, currentChar) ^ contains(alphabets, lastChar)) || (lastChar == '(' && currentChar == ')') || (lastChar == '{' && currentChar == ')') || (currentChar == ';') || (lastChar == ')' && currentChar == '{') || (contains(alphabets_2, currentChar) ^ contains(alphabets_2, lastChar)) || (isAlphabet(lastChar) ^ isAlphabet(currentChar)) || (isValidInteger(currentChar) ^ isValidInteger(lastChar)) )){
            string tokenClass = dfa->getStateById(currentStateID)->getTokenClassForFinalStates();
            ...
            currentStateID = dfa->isNextStatePresentForGivenInput(currentStateID, currentChar);
            // i--;
            //cout<<"aaa\n";
        // }
